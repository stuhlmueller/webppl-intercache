...
var prevalenceERPobject = _.object(map(function(p){
  var categories = _.uniq(_.pluck(subset(df_truth, "Property", p), "Category"))
  return [p, _.object(map(function(k){
      var fpath = "data/prevalence_"+p+"_"+k+".csv"
...
var priorERPobject = _.object(map(function(p){
    var fpath = "data/prevalence_"+p+"_MH50kb.csv"
    var df = readQueryERP(fpath)
    return [p, df]
},properties))
...
console.log('before before extract prev info')
...
var prevalence = genUtils.closest(bins,prevalenceERPobject["are full-grown"]["Swans"].MAP().val.Prevalence)

var priorMAP = priorERPobject["are full-grown"].MAP().val
var prior = discretizedPriorModel(priorMAP.Theta, priorMAP.Gamma, priorMAP.Delta)
...
console.log('after score speaker')
